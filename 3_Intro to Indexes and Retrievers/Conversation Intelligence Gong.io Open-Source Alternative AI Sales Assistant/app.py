...
def load_db(self):
    ...
    return DeepLake(dataset_path=f'deeplake/{self.file_name}', embedding_function=CohereEmbeddings(), read_only=True)  
...
def create_db(self):
    ...
    return DeepLake.from_texts(self.data, CohereEmbeddings(), dataset_path=f'deeplake/{self.file_name}')
...
def query_db(self, query):
    ...
    Query the database for passenges that are similar to the query.
